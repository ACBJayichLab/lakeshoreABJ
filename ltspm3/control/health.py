...
class SensorGuardConfig:
    ...
    * genuine cooldown transients reach **1.63 K/s** (-6.5 K in one 4 s sample,
    ...
    #: outage the cryostat may genuinely have moved -- so a stale reference must
...
class SensorGuard:
    ...
    def _check(
        ...
        # dropout the cryostat really may have moved, so a stale reference must not
        ...
        if self.last_good_k is not None and self.last_good_t is not None:
            ...
            if self.cfg.min_interval_s <= age <= max_age:
                ...
                # than making a single-sensor cryostat uncontrollable.
