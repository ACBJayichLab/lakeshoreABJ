...
class ExponentialFilter:
    ...
    At tau=60 s and dt=4 s the output noise is 0.18x the input, i.e. the rig's
    ~10 mK sample noise becomes ~1.8 mK -- enough for millikelvin work while
    staying 6x faster than the plant's ~360 s fast pole.
    """
...
class MeasurementFilter:
    ...
    def __init__(
        ...
        *,
        ...
        #: a plant that has since moved, so :meth:`predict` is no longer a
    ...
    def reseed(self, t: float, value: float) -> None:
        ...
        Called when control resumes after an outage long enough that the plant
    ...
    def noise_estimate(self) -> float:
        ...
        temperature -- on this rig it is ~1 mK at 18 K and ~10 mK at 96 K.
        """
    ...
    def predict(self, dt: float) -> float | None:
        ...
        On the real rig's commanded ramp (2.5 K per sample) that would drive the
