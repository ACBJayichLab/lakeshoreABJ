...
| | |
|---|---|
| [`docs/recorder/`](docs/recorder/) | **Generic, any rig.** [install](docs/recorder/install.md) · [quickstart](docs/recorder/quickstart.md) · [cli](docs/recorder/cli.md) · [configuration](docs/recorder/configuration.md) · [instruments](docs/recorder/instruments.md) · [file-interface](docs/recorder/file-interface.md) · [gui](docs/recorder/gui.md) · [windows](docs/recorder/windows.md) · [troubleshooting](docs/recorder/troubleshooting.md) |
| [`docs/ltspm/`](docs/ltspm/) | **LTSPM3 only.** [rig](docs/ltspm/rig.md) · [safety](docs/ltspm/safety.md) · [plant](docs/ltspm/plant.md) · [control](docs/ltspm/control.md) · [running](docs/ltspm/running.md) |
...
**Keep the split when you write.** Anything true of any Lake Shore rig belongs
in `docs/recorder/`; anything calibrated to LTSPM3 belongs in `docs/ltspm/`. A
...
1. **`lschart` never imports `ltspm`.** If you find yourself wanting it to, the
...
5. **Writes are applied asynchronously** — a query issued too soon answers with
   ...
   GPIB; check `verify_readback` before the LTSPM rig runs armed.**
   → [instruments](docs/recorder/instruments.md)
6. **Availability of the cryostat outranks control quality.** Every ambiguous
   ...
   [safety](docs/ltspm/safety.md) and are not negotiable.
...
9. **Where a measured number contradicts memory, the number wins.** Re-derive
   from `reference/logs/`, don't trust the prose. → [plant](docs/ltspm/plant.md)
...
matlab/              LakeShore.m -- MATLAB's half of the file protocol, plus
                     selftest.m and a README. Not built; copied to the rig.
docs/                recorder/ (generic) and ltspm/ (one rig). Keep them apart.
