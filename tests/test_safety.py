...
def test_large_error_is_treated_as_a_broken_premise_not_a_command(harness):
    """A large error that was *not* commanded means the rig is wrong, not the loop.
...
def test_instrument_rdgst_fault_is_believed(harness):
    h = armed(harness)
    before = h.sup.output_pct
    h.rig.inject(rdgst_channels={"218.1": 32})     # temp overrange
    st = h.step(2)
