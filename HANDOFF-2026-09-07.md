...
> **Superseded.** This was the live handoff until 2026-09-10 and is kept for
> the sweep-tool history in it. Current status is [HANDOFF.md](HANDOFF.md); the
> refit's state is [REFIT_PLAN.md](REFIT_PLAN.md). Its anchor counts (92) and
> its `analysis/steps.py` commands are both obsolete -- the archive and
> `analysis/measure.py` replaced them.
