...
:mod:`lschart.instruments.sim`, which knows nothing about this rig.  The model
...
Build a rig with :func:`ltspm_rig`, which wires this plant and the measured
cross-channel couplings into the generic :class:`~lschart.instruments.sim.SimulatedRig`.
"""
...
from lschart.instruments.sim import SimulatedRig
...
def ltspm_rig(params: PlantParams | None = None, *, start_k: float = 96.0, **kw):
    """A :class:`SimulatedRig` running the calibrated LTSPM plant."""
    return SimulatedRig(
        plant=ThermalModel(params, start_k=start_k),
