...
| Coldplate / Magnet | 8.47 K / 7.04 K — **the Coldplate figure is pre-calibration**: it was logged on X186276's curve and the thermometer is X186279, so the coldplate was at **6.89 K**, not 8.47. See [cryostat](cryostat.md#the-coldplate-was-reading-high-because-it-had-another-sensors-curve) |
...
| Coldplate recalibrated | **2026-09-04 12:07:16.** Input 2 was carrying another thermometer's curve -- X186276's, where the Coldplate is X186279 -- so it read high by 12-13% of absolute temperature for as long as that curve was loaded. Every Coldplate number above, and in every log before the cutover, carries it. The correction is exactly computable, both curves are in `reference/sensor-curves/`, and the pre-cutover logs have been remapped into `data/coldplate-recal/`. See [cryostat](cryostat.md) |
