...
the plant's ~620 s pole low-passes the dither to sub-mK ripple.

### One subtlety the quadratic actuator introduces

The dither averages *voltage*, but the plant responds to *power*, and
...
for (`tests_ltspm/test_plant.py`) so nobody has to rediscover it while chasing
an offset.

## Why the model is in two stages

`ltspm/plant.py` deliberately keeps **`P(pct)`** and **`T(P)`** apart, and both
...
| Module | |
|---|---|
| `ltspm/plant.py` | the one measured `P(pct)` / `T(P)` curve |
| `ltspm/sim_plant.py` | two-pole calibrated model + measured cross-channel coupling |
