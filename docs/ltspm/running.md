# Running the LTSPM loop

`ltspm` is a thin shim over the `lschart` CLI — it swaps what builds the
...
python -m ltspm -c config.yaml run                      # records; loop NOT closed
...
PID and the filter are primed **bumplessly** from what the rig is doing now. If
...
**Check this before the LTSPM rig runs armed.**

### 2. Nothing on this rig has been talked to yet
...
`ltspm/tools/steptest.py` holds the protocol.
...
.venv/bin/python -m pytest -q tests_ltspm     # the control half
```

`tests_ltspm/conftest.py` carries the virtual-clock harness: time is injected,
...
- **Is the noise model right?** The bench 336 reads 0.44–3.03 mK rms at ~296 K
  where [plant.md](plant.md) claims 109 mK at 290 K for the 218 sample channel.
