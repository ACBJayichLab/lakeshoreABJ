...
calibrated to Jeff's LTSPM3 rig and does not transfer. If you are running a
...
That is the whole reason `ltspm` exists as a separate package. Every other rig
...
ltspm     LTSPM3 only          IS the loop, on the 218's analog output
```

`ltspm` imports `lschart`. Nothing in `lschart` may import `ltspm`.

## Read in this order

1. **[rig.md](rig.md)** — the hardware, the addresses, what is wired to what,
...
3. **[plant.md](plant.md)** — what the cryostat actually does, measured over
   1,510 hours of logs.
...
`ltspm` is **complete, tested and parked**. The stated priority is the recorder,
the GUI and the MATLAB interface; resist "while I am in here" improvements to
`control/`.

Two things are worth knowing before it runs against the real rig:

- **Nothing on this rig has been talked to yet.** Every number in these
