...
[plant.md](plant.md) is where every default number comes from.
...
**Feedforward and the simulator import the same curve.** `ltspm/plant.py` is the
single copy of `P(pct)` and `T(P)`, so the model the controller assumes and the
model the simulator implements cannot drift apart. Testing against a plant that
...
the operating point. See [plant.md](plant.md#the-consequence-that-shapes-the-whole-design).
...
`ltspm/config.py` registers the `control:` section on import, which is why
