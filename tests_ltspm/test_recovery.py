...
PID bias when control resumed, even though the plant and the output had both
...
def test_guard_escapes_fault_after_the_plant_has_moved(harness):
    ...
    freezes while the plant keeps moving -- and a fault ramp-down guarantees the
    plant moves.  On recovery every honest reading sat far from the stale
    ...
    """
    h = armed(harness)
    h.rig.inject(dropout_channels={"218.1"})
    ...
    assert h.sup.output_pct < 63.076              # the plant is now cooling
...
def test_filter_reseeds_rather_than_rejecting_forever(harness):
    ...
    # The reseeded filter must describe the plant *now*, not the pre-fault value.
    # It still lags a falling plant, which is what a low pass is for -- what
...
def test_recovery_does_not_ratchet_further_down(harness):
    """A ramp-down cools the plant, which grows the error, which used to trigger
