...
| `band.py` | **how wrong the residual is allowed to be**, and PID phase 1's gate. Measures the six terms of `σ_Q` — the coldplate's own residual through `Λ′(T_c)`, the median anchor bar, the diurnal amplitude, the in-epoch model error, the campaign drift and `σ_C·|dT/dt|` — and `export_response.py` freezes them into `ltspm3/model/_fitted_table.py`. **Two error bars, and keeping them apart is the design**: `sigma_q_w` is a noise band and `bias_q_w` is where the level may sit; the 0.7 % the heater circuit may not deliver is a constant, and in the band it makes 3σ at 118 K 14 mW against the −4.9 mW the monitor has to catch. 3σ settled comes out at 1.4 mW at 118 K, which is 0.87 K — the band is about a kelvin everywhere, arrived at from measured terms rather than chosen. Also prints `δQ` at every settled anchor, in-epoch and across the campaign, in mW **and** in K, because a milliwatt gate is a kelvin gate divided by `Λ′` and `Λ′` runs 24 mW/K at 10 K against 1.7 at 118 K |
...
| `pid_tuning.py` | SIMC PI gains scheduled against T, on the loop *as configured* — a 60 s measurement filter slower than the cryostat below 60 K, and the half rule needed to describe two comparable lags. **`--rows`** is the other question: the schedule phase 3 §3.2 rebuilds the loop on, from the PRODUCTION fit, every 10 K, with the delay DERIVED from the filter configuration and the fit's cache key on every row. The delay floor binds in `move` at 10–30 K, where the cryostat's own τ is under 24 s |
