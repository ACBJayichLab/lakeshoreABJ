...
| **4a-i** | here | **the smoothed setpoint says when it has ARRIVED, in kelvin** — `RampConfig.settled_k`, 2026-09-18. It was an underflow test on the rate, so the status file read `ramping`, the velocity feedforward kept a decayed tail, and **rule 4's kelvin premise rows stayed switched off, for ~7 min after every move**. Now ~2 min. **The HOLD/MOVE gain switch is quarantined on the old test** and still waits: relaxing `kp` by 7.1× mid-convergence costs tens of mK, and a bench sweep over 380–480 s passes and fails in no order, so the delay is not a number to tune — the fix is to ramp the gain change rather than step it, and that is not done. **Superseded by 4a-ii**: the step was not the problem, the output it kept was |
| **4a-ii** | here | **`settled` is the switch to the hold gains, and the handover keeps the average output** — 2026-09-23. The quarantine in 4a-i was hiding the real cost: `PID.set_gains` held P + I across the switch, which inside the gate is the last cycle's sensor noise times the move `kp` (output 34 m% rms, 0.87 correlated with the previous reading — noise, not ringing). On the cryostat that froze 64.08 % into a loop jittering around 64.10, and the sample sat 45–66 mK low for 7+ min. The MOVE → HOLD handover now keeps `ki·I` (`keep="integral"`), the switch follows the 5 mK arrival test, and `rate_underflowed` is gone. Bench: every graded move stays inside 50 mK from the verdict on, which comes 234–418 s after the command instead of 530–680 s. One `settled` flag in the status file is what MATLAB and the viewer read, and **it stays true**: once given it ends only after `unsettle_s` (30 s) outside the gate. Replayed through the shipped `Tuner` over every armed hold 09-18..23 (122 h at target) it is true 99.2 % of that time, unbroken for whole days, with two drop-outs, both real — 09-22 08:40 (54 s at up to 80 mK) and the 09-23 12:31 sag this step removes the cause of. Without the 30 s it was 18, 17 of them one 10 mK quantum past the gate for 2–8 s. **Open: 180 K** — the Sample noise there is near the 50 mK gate and the 120 s entry dwell never completes. Cryostat check pending |
| **4a-iii** | here | **the onset of a fast move is not a sensor glitch** — 2026-09-23. Every 2 K move froze the heater for 14 s at its overdrive, 14–22 s in, up and down (worse up: the heater kicks the sample harder than cooling pulls it). The spike test predicted on a 30 s slope that lags the sample going from rest to 3.3 K/min, missed by ~0.19 K against a 0.16 K threshold (8 × the 20 mK floor), rejected two honest readings, and the guard wanted five good ones back. `filter.spike_min_k: 0.40` is the smallest jump the spike test may reject, and touches nothing else — raising `spike_floor_k` instead also narrowed the residual's `slope_lag` band, and raising `spike_sigma` let 6 of the archive's 29 real glitches through during noisy cooldowns. Replayed: today's 4 freezes → 0 (closest call 0.63 of threshold), no rejection in 4½ days of armed holds, and over the 63-day archive all 29 glitches still caught, 0 in FAULT, 3.97 → 2.50 rejections/day. Cryostat check pending |
